...
exports.psqlErrorHandler = (err, req, res, next) => {
	if (err.code && err.code === "22P02") {
		res.status(400).send({
			msg: "Bad data type. Reconsider path requirements.",
		});
		...
		res.status(404).send({
			msg: "Bad post request. Reconsider provided body.",
		});
	} else if (err.code && err.code.length === 5) {
...
exports.defaultErrorHandler = (err, req, res, next) => {
	...
	res.status(500).send({
		msg: "INTERNAL SERVER ERROR. Examine code and resolve error.",
	});
};
